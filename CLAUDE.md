...
- **Deploy pipeline**: push to `main` → `.github/workflows/deploy.yml` POSTs to `/api/webhooks/deploy` with a `DEPLOY_SECRET` bearer token → `scripts/deploy.mjs` on the VPS does `git worktree add` for the new commit under `/opt/aiva/releases/<sha>`, `pnpm install --frozen-lockfile`, `prisma generate`, `pnpm --filter web run build`, then repoints the `/opt/aiva/current` symlink and runs `pm2 restart ai-chat-web`. Deploy log: `/opt/aiva/ops/deploy.log` on the VPS.
